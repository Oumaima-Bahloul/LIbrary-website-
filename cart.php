...
<body class="d-flex flex-column min-vh-100">
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // ---- FETCH YOUR PRODUCTS ----
        $items_stmt = $pdo->prepare("SELECT c.*, p.title, p.price FROM cart c JOIN products p ON c.product_id = p.id WHERE c.user_id = ?");
    ...
    <div class="container" style="margin-top: 150px; margin-bottom: 80px;">
        ...
        <?php else: ?>
            ...
            $shipping_limit = 100000;
            $shipping_fee = 8000;
            $shipping = ($grand_total >= $shipping_limit) ? 0 : $shipping_fee;
            $final_total = $grand_total + $shipping;
            ?>
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                            ...
                            <table class="table align-middle mb-0">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            <td class="ps-4 py-4">
                                                <h6 class="mb-0 fw-bold text-capitalize"><?php echo htmlspecialchars($item['title']); ?></h6>
                                            </td>
                                            <td class="text-center"><?php echo number_format($item['price']); ?> TND</td>
                                            <td class="text-center">
                                                <div class="input-group input-group-sm justify-content-center" style="width: 120px; margin: auto;">
                                                    <a href="manage_cart.php?action=decrease&id=<?php echo $item['product_id']; ?>" class="btn btn-outline-secondary px-2">-</a>
                                                    <input type="text" class="form-control text-center" value="<?php echo $item['quantity']; ?>" readonly>
                                                    <a href="manage_cart.php?action=increase&id=<?php echo $item['product_id']; ?>" class="btn btn-outline-secondary px-2">+</a>
                                                </div>
                                            </td>
                                            <td class="text-center fw-bold text-primary"><?php echo number_format($subtotal); ?> TND</td>
                                            <td class="pe-4 text-end">
                                                <a href="manage_cart.php?action=remove&id=<?php echo $item['product_id']; ?>" class="text-muted">
                    ...
                    <div class="mt-3">
                        <a href="index.php" class="btn btn-link text-decoration-none text-muted p-0 border-0">
                ...
                <div class="col-lg-4">
                    <div class="card shadow-sm border-0 p-4">
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span class="fw-bold"><?php echo number_format($grand_total); ?> TND</span>
                        </div>

                        <div class="d-flex justify-content-between mb-3 pb-3 border-bottom">
                            <span class="text-muted">Delivery:</span>
                            <span class="fw-bold <?php echo ($shipping == 0) ? 'text-success' : 'text-danger'; ?>">
                                <?php echo ($shipping == 0) ? 'Free' : number_format($shipping) . ' TND'; ?>
                            </span>
                        </div>

                        <?php if ($shipping > 0): ?>
                            <div class="alert alert-warning small py-2 border-0 mb-3">
                                Add <b><?php echo ($shipping_limit - $grand_total); ?> TND</b> for <b>Free Delivery!</b>
                            </div>
                        <?php endif; ?>

                        <div class="d-flex justify-content-between mb-4">
                            <span class="h5 mb-0 fw-bold">Total:</span>
                            <span class="h5 mb-0 fw-bold text-primary"><?php echo number_format($final_total); ?> TND</span>
                        </div>

                        <button onclick="window.location.href='checkout.php'" class="btn btn-dark w-100 py-3 fw-bold rounded-pill shadow-sm">
                            Confirm Order
                        </button>
                    </div>
                </div>
